...
void GazeboMotorModel::Publish() {
  ...
  //Publish function to take longer to execute, so the motor updates will be slower
...
void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if(_sdf->HasElement("reversible")) {
    ...
    //double latency = 1.0;  // 5 milliseconds
  ...
  */
  ...
  // Initialize the timer and start scheduling updates
  //timer_ = gazebo::event::Events::ConnectWorldUpdateBegin(
  //    std::bind(&GazeboMotorModel::OnUpdate, this));
...
void GazeboMotorModel::UpdateForcesAndMoments() {
  ...
  double desired_velocity = turning_direction_ * ref_motor_rot_vel / rotor_velocity_slowdown_sim_;
  // Add the current motor velocity command to the buffer
  motor_velocity_buffer_.push_back(desired_velocity);
  // Retrieve the oldest command from the buffer, corresponding to the latency time
  joint_->SetVelocity(0, motor_velocity_buffer_.front());
  // Add the current motor velocity command to the buffer
  motor_velocity_buffer_.pop_front();
